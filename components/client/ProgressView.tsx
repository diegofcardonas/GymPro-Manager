    ...
    return (
        <div className="w-full max-w-6xl space-y-8 animate-fade-in pb-20">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight">Mi Progreso</h2>
                    <p className="text-gray-500 dark:text-gray-400 font-medium">Visualiza tu evolución y celebra tus victorias.</p>
                </div>
            ...
            {workoutHistory.length > 0 ? (
                <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-black/5">
                            <div className="flex justify-between items-start mb-4">
                                ...
                                <span className="text-[10px] font-black uppercase text-gray-400">Peso Máx</span>
                            </div>
                            <p className="text-3xl font-black text-gray-900 dark:text-white">{personalBests.weight} <span className="text-sm text-gray-400">kg</span></p>
                        </div>
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-black/5">
                            <div className="flex justify-between items-start mb-4">
                                <div className="p-3 bg-emerald-500/10 text-emerald-500 rounded-2xl"><FireIcon className="w-6 h-6"/></div>
                                <span className="text-[10px] font-black uppercase text-gray-400">Volumen Máx</span>
                            </div>
                            <p className="text-3xl font-black text-gray-900 dark:text-white">{personalBests.volume.toLocaleString()} <span className="text-sm text-gray-400">kg</span></p>
                        </div>
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-black/5">
                            <div className="flex justify-between items-start mb-4">
                                <div className="p-3 bg-purple-500/10 text-purple-500 rounded-2xl"><ChartBarIcon className="w-6 h-6"/></div>
                                <span className="text-[10px] font-black uppercase text-gray-400">Est. 1RM</span>
                            </div>
                            <p className="text-3xl font-black text-gray-900 dark:text-white">{personalBests.e1rm} <span className="text-sm text-gray-400">kg</span></p>
                        </div>
                        <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-black/5">
                            <div className="flex justify-between items-start mb-4">
                                <div className="p-3 bg-orange-500/10 text-orange-500 rounded-2xl"><CalendarDaysIcon className="w-6 h-6"/></div>
                                <span className="text-[10px] font-black uppercase text-gray-400">Total Sesiones</span>
                            </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            <div className="bg-white dark:bg-gray-800 p-8 rounded-4xl shadow-sm border border-black/5">
                                <h3 className="text-xl font-black mb-8 text-gray-900 dark:text-white flex items-center gap-2">
                                    <ChartBarIcon className="w-6 h-6 text-primary" /> Tendencia de Fuerza
                                </h3>
                                <div className="h-[400px]">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                            ...
                                            />
                                            <Area type="monotone" dataKey="maxWeight" name="Peso Máx" stroke="hsl(var(--primary))" fill="url(#colorMax)" strokeWidth={4} dot={{ r: 4, fill: 'hsl(var(--primary))', strokeWidth: 2, stroke: '#fff' }} />
                                            <Area type="monotone" dataKey="estimated1RM" name="Estimado 1RM" stroke="#a855f7" fill="transparent" strokeWidth={2} strokeDasharray="5 5" />
                                        </AreaChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
                            ...
                            <div className="bg-white dark:bg-gray-800 p-8 rounded-4xl shadow-sm border border-black/5">
                                <h3 className="text-xl font-black mb-8 text-gray-900 dark:text-white flex items-center gap-2">
                                    <FireIcon className="w-6 h-6 text-emerald-500" /> Volumen de Entrenamiento
                                </h3>
                                <div className="h-[300px]">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                            ...
                                            <Line type="step" dataKey="totalVolume" name="Volumen Total" stroke="#10b981" strokeWidth={3} dot={false} />
                                        </LineChart>
                                    </ResponsiveContainer>
                                </div>
                            </div>
                        ...
                        <div className="space-y-8">
                            <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-black/5">
                                <h3 className="text-sm font-black uppercase tracking-widest text-gray-400 mb-6">Consistencia Mensual</h3>
                            ...
                            <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-sm border border-black/5 flex flex-col max-h-[600px]">
                                <h3 className="text-sm font-black uppercase tracking-widest text-gray-400 mb-6">Historial Reciente</h3>
                                <div className="space-y-4 overflow-y-auto pr-2 custom-scrollbar">
                                    {[...workoutHistory].reverse().map(session => (
                                        <div key={session.id} className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-900/30 border border-black/[0.02] dark:border-white/[0.02]">
                                            ...
                                            <p className="text-sm font-bold text-gray-800 dark:text-gray-200">{session.loggedExercises.length} ejercicios registrados</p>
                ...
                <div className="py-20 flex flex-col items-center justify-center text-center bg-white dark:bg-gray-800 rounded-4xl border-2 border-dashed border-black/5 opacity-50">
                    ...
                    <h3 className="text-2xl font-black text-gray-400">Aún no hay datos</h3>
                    <p className="text-gray-400 mt-2 max-w-sm">Registra tu primer entrenamiento hoy para empezar a construir tu gráfica de progreso.</p>
                </div>
            )}
        </div>
    );
