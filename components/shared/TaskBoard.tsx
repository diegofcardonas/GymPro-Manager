...
const TaskBoard: React.FC = () => {
    ...
    // Simular carga asíncrona de datos del contexto
    useEffect(() => {
    ...
    if (myTasks.length === 0) {
        return (
            <div className="bg-white dark:bg-gray-800 p-12 rounded-4xl text-center border border-black/5 opacity-50 flex flex-col items-center animate-fade-in">
        ...
        <div className="space-y-4 animate-fade-in">
            ...
            <div className="space-y-4">
                {myTasks.map(task => (
                    <div 
                        key={task.id} 
                        className={`bg-white dark:bg-gray-800 p-5 rounded-3xl shadow-sm border border-black/5 group flex items-start gap-4 transition-all hover:shadow-md hover:-translate-y-0.5 ${task.status === TaskStatus.COMPLETED ? 'opacity-75' : ''}`}
                    >
                        <button 
                            onClick={() => handleToggleStatus(task)}
                            className={`mt-1 p-1 rounded-xl transition-all transform active:scale-90 hover:scale-110 duration-200 shadow-sm
                                ${task.status === TaskStatus.COMPLETED 
                                    ? 'bg-green-500 text-white shadow-green-500/20 rotate-0' 
                                    : 'bg-gray-50 dark:bg-gray-700 text-gray-300 hover:text-primary'
                                }`}
                        >
                            <CheckCircleIcon className={`w-6 h-6 transition-transform duration-300 ${task.status === TaskStatus.COMPLETED ? 'scale-100' : 'scale-75'}`} />
                        </button>
                        
                        <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-start mb-1 gap-2">
                                <h4 className={`font-bold text-base truncate transition-all duration-300 ${task.status === TaskStatus.COMPLETED ? 'line-through text-gray-400' : 'text-gray-900 dark:text-white'}`}>
                                    {task.title}
                                </h4>
                                <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase flex-shrink-0
                            ...
                            <div className="flex items-center gap-2 mt-3 text-[10px] font-black text-gray-300 uppercase tracking-tighter">
                                <div className={`p-1 rounded-md ${new Date(task.dueDate) < new Date() && task.status !== TaskStatus.COMPLETED ? 'text-red-400' : ''}`}>
                                ...
                                <span className={task.status === TaskStatus.IN_PROGRESS ? 'text-primary' : ''}>
                                    {task.status}
                                </span>
                            </div>
                        </div>
            ...
            <style>{`
                ...
                .status-active-animation {
                    animation: task-pop 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);
                }
            `}</style>
        </div>
    );
