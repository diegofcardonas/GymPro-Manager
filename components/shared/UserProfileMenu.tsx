...
export const UserProfileMenu: React.FC<UserProfileMenuProps> = ({ user, onEditProfile, onSettings, onLogout }) => {
    ...
    return (
        <div className="relative" ref={menuRef}>
            ...
            >
                <span className="hidden sm:inline font-medium text-gray-700 dark:text-gray-300">{t('common.welcome', { name: user.name })}</span>
                <img 
                    src={user.avatarUrl} 
                    alt="Profile" 
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-2xl ring-1 ring-black/5 dark:ring-white/10 py-2 z-50 animate-scale-in origin-top-right">
                    ...
                    {onEditProfile && (
                        <button
                            ...
                            className="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-3"
                        >
