        ...
        <div className="w-full max-w-6xl mx-auto space-y-6 animate-fade-in pb-12">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-500 font-medium">Asigna y supervisa las tareas del equipo.</p>
                </div>
                <button onClick={() => { setEditingTask(null); setIsModalOpen(true); }} className="px-6 py-3 bg-primary text-white rounded-2xl font-black shadow-lg shadow-primary/20 hover:scale-105 transition-all flex items-center gap-2">
                ...
                {[TaskStatus.PENDING, TaskStatus.IN_PROGRESS, TaskStatus.COMPLETED].map(status => (
                    <div key={status} className="bg-gray-100 dark:bg-gray-900/50 p-4 rounded-3xl min-h-[500px] flex flex-col border border-black/5 dark:border-white/5">
                        <div className="flex items-center justify-between mb-4 px-2">
                            ...
                            <span className="bg-white dark:bg-gray-800 px-2 py-1 rounded-lg text-[10px] font-black">{tasks.filter(t => t.status === status).length}</span>
                        </div>
...
const TaskCard: React.FC<{ task: Task; users: User[]; onEdit: () => void; onDelete: () => void }> = ({ task, users, onEdit, onDelete }) => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-black/5 dark:border-white/5 group relative transition-all hover:shadow-md">
            <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={onEdit} className="p-1 hover:text-primary transition-colors"><PencilIcon className="w-4 h-4"/></button>
                    <button onClick={onDelete} className="p-1 hover:text-red-500 transition-colors"><TrashIcon className="w-4 h-4"/></button>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between border-t border-gray-50 dark:border-gray-700/50 pt-3">
                <div className="flex items-center gap-2">
                    <img src={assignee?.avatarUrl} className="w-6 h-6 rounded-full border border-white" />
                    <span className="text-[10px] font-bold text-gray-400">{assignee?.name.split(' ')[0]}</span>
                </div>
                <span className="text-[10px] font-black text-gray-300">{new Date(task.dueDate).toLocaleDateString()}</span>
            </div>
        </div>
    );
};
        ...
        <div className="fixed inset-0 z-[60] bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
            <div className="bg-white dark:bg-gray-800 rounded-4xl w-full max-w-lg shadow-2xl overflow-hidden animate-scale-in">
                <div className="p-8 border-b border-black/5">
                    <h2 className="text-2xl font-black">{task ? 'Editar Tarea' : 'Nueva Tarea'}</h2>
                </div>
                <div className="p-8 space-y-6">
                    <input 
                        type="text" placeholder="Título de la tarea" 
                        value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})}
                        className="w-full p-4 bg-gray-50 dark:bg-gray-900 border-none rounded-2xl focus:ring-2 focus:ring-primary font-bold"
                    />
                    <textarea 
                        placeholder="Descripción detallada..." 
                        value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})}
                        className="w-full p-4 bg-gray-50 dark:bg-gray-900 border-none rounded-2xl focus:ring-2 focus:ring-primary text-sm min-h-[100px]"
                    />
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-[10px] font-black uppercase text-gray-400 ml-1">Asignar a</label>
                            <select 
                                value={formData.assignedToId} onChange={e => setFormData({...formData, assignedToId: e.target.value})}
                                className="w-full p-3 bg-gray-50 dark:bg-gray-900 border-none rounded-xl mt-1 text-sm font-bold"
                            >
                ...
                <div className="p-8 bg-gray-50 dark:bg-gray-900/50 flex gap-4">
                    <button onClick={onClose} className="flex-1 p-4 rounded-2xl font-black text-gray-400 hover:bg-gray-200 transition-colors uppercase text-xs tracking-widest">Cancelar</button>
                    <button onClick={() => onSave(formData)} className="flex-1 p-4 bg-primary text-white rounded-2xl font-black shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all uppercase text-xs tracking-widest">Guardar Tarea</button>
                </div>
            </div>
        </div>
    );
