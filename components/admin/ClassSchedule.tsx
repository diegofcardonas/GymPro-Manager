...
const ClassSchedule: React.FC = () => {
    ...
    const handleDelete = (id: string, e?: React.MouseEvent) => {
        e?.stopPropagation();
        // e?.nativeEvent.stopImmediatePropagation(); // Removed to be safer with React events
        setClassToDelete(id);
    };
    ...
    const handleMoveClass = (classId: string, newDateStr: string) => {
        const classToMove = gymClasses.find(c => c.id === classId);
        if (!classToMove) return;

        const oldStart = new Date(classToMove.startTime);
        const oldEnd = new Date(classToMove.endTime);
        const durationMs = oldEnd.getTime() - oldStart.getTime();

        // Parse target date components
        // We use split/map to avoid timezone shifts often caused by new Date(string)
        const [year, month, day] = newDateStr.split('-').map(Number);

        // Create new start date using target date but ORIGINAL time
        const newStartTime = new Date(year, month - 1, day, oldStart.getHours(), oldStart.getMinutes(), 0, 0);
                ...
                <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
                    ...
                    <div className="flex items-center bg-white dark:bg-gray-800 p-1 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                        <button 
                            ...
                            title={t('classSchedule.calendar')}
                        >
                    ...
                    <div className="relative flex-grow lg:flex-grow-0 min-w-[200px]">
                        ...
                        <select
                            ...
                            className="block w-full pl-10 pr-4 py-2.5 text-sm bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl focus:ring-primary focus:border-primary text-gray-900 dark:text-white shadow-sm appearance-none cursor-pointer"
                        >
                    ...
                    <div className="divide-y divide-gray-200 dark:divide-gray-700">
                        {filteredClasses.length > 0 ? filteredClasses.map(gymClass => {
                            ...
                            <div className="p-12 text-center flex flex-col items-center justify-center">
                                ...
                                <p className="text-gray-500 dark:text-gray-400 text-lg">{t('booking.noClasses')}</p>
                                <button onClick={() => handleAddNew()} className="mt-4 text-primary hover:underline font-medium">{t('general.create')}</button>
                            </div>
                        )}
                    </div>
                )}
...
const CalendarView: React.FC<{ 
    ...
    onClassMove: (classId: string, newDateStr: string) => void;
}> = ({ classes, trainers, onDateClick, onClassClick, onDelete, onClassMove }) => {
    const { t } = useTranslation();
    ...
    const handleDragStart = (e: React.DragEvent, classId: string) => {
        ...
        // Optional: Set ghost image or style here if needed
    };
    ...
    const handleDragOver = (e: React.DragEvent, dateStr: string) => {
        ...
        // Optimize: Only update state if changed to avoid renders
    ...
    const handleDragLeave = (e: React.DragEvent) => {
        // We only clear if we are leaving the current cell entirely.
        // However, checking currentTarget vs relatedTarget is complex with child elements.
        // A simpler strategy is to only rely on DragOver setting the active one,
        // and Drop/End clearing it.
    };
    ...
    const handleDrop = (e: React.DragEvent, dateStr: string) => {
        ...
        if (classId) {
            onClassMove(classId, dateStr);
        }
    };

    // Clean up drag state if drag ends anywhere
    const handleDragEnd = () => {
        setDragOverDate(null);
    }

    return (
        <div className="flex flex-col h-full select-none" onDragEnd={handleDragEnd}>
            {/* Calendar Header */}
            <div className="flex items-center justify-between p-4 md:p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/30">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center bg-white dark:bg-gray-700 rounded-lg shadow-sm border border-gray-200 dark:border-gray-600 p-0.5">
                        ...
                        <button onClick={goToToday} className="px-3 py-1 text-xs font-bold uppercase tracking-wider hover:bg-gray-100 dark:hover:bg-gray-600 rounded-md transition-colors">{t('classSchedule.today')}</button>
            ...
            <div className="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
                {weekDays.map(day => (
                    <div key={day} className="py-3 text-center text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest truncate">
                        {day.substring(0, 3)}
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-7 flex-1 auto-rows-fr bg-gray-200 dark:bg-gray-700 gap-px border-b border-gray-200 dark:border-gray-700">
                {days.map((d, index) => {
                    // Create date string manually to avoid timezone offset issues when rendering
                    const monthStr = String(d.month + 1).padStart(2, '0');
                    const dayStr = String(d.day).padStart(2, '0');
                    const isoDateStr = `${d.year}-${monthStr}-${dayStr}`;
                    
                    const isToday = isoDateStr === new Date().toISOString().split('T')[0];
                    const isDragOver = dragOverDate === isoDateStr;
                    
                    const dayClasses = classes.filter(c => {
                        return c.startTime.startsWith(isoDateStr);
                    ...
                    return (
                        <div 
                            key={index} 
                            onClick={() => onDateClick(isoDateStr)}
                            onDragOver={(e) => handleDragOver(e, isoDateStr)}
                            onDrop={(e) => handleDrop(e, isoDateStr)}
                            className={`min-h-[120px] group bg-white dark:bg-gray-800 p-2 transition-all cursor-pointer flex flex-col gap-1 relative
                                ${!d.isCurrentMonth ? 'bg-gray-50/50 dark:bg-gray-900/50 text-gray-400' : ''}
                                ${isDragOver ? 'bg-blue-50 dark:bg-blue-900/30 ring-2 ring-inset ring-primary z-10' : 'hover:bg-gray-50 dark:hover:bg-gray-750'}
                            `}
                        >
                            ...
                            <div className="flex-1 flex flex-col gap-1.5 mt-1 overflow-hidden">
                                {dayClasses.slice(0, maxVisible).map((cls) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            className="text-left px-2 py-1 rounded-md text-[10px] lg:text-xs font-semibold truncate border shadow-sm transition-transform hover:scale-[1.02] active:scale-95 w-full relative overflow-hidden flex justify-between items-center group/item cursor-grab active:cursor-grabbing"
                                            style={{ backgroundColor: bg, color: text, borderColor: border }}
                                        ...
                                        >
                                            ...
                                            <button 
                                                ...
                                                className="ml-1 p-0.5 rounded hover:bg-red-500 hover:text-white text-red-600 opacity-0 group-hover/item:opacity-100 transition-opacity md:opacity-0 md:group-hover/item:opacity-100 opacity-100"
                                            >
                                ...
                                {overflowCount > 0 && (
                                    <div className="text-[10px] font-bold text-gray-500 dark:text-gray-400 text-center bg-gray-100 dark:bg-gray-700 rounded-md py-0.5 pointer-events-none">
                                        +{overflowCount} {t('classSchedule.more')}
                                    </div>
                                )}
                            </div>
                        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg animate-scale-in overflow-hidden">
                <div className="p-6 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                    <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{gymClass ? t('classSchedule.modal.edit') : t('classSchedule.modal.new')}</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400">{t('classSchedule.modal.desc')}</p>
                </div>
                <div className="p-6 space-y-5">
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('classSchedule.modal.name')}</label>
                        <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded-lg p-2.5 focus:ring-primary focus:border-primary text-gray-900 dark:text-white" placeholder={t('classSchedule.modal.namePlaceholder')} required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('classSchedule.modal.description')}</label>
                        <textarea name="description" value={formData.description} onChange={handleChange} rows={3} className="w-full bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded-lg p-2.5 focus:ring-primary focus:border-primary text-gray-900 dark:text-white" placeholder={t('classSchedule.modal.descPlaceholder')} />
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('classSchedule.modal.trainer')}</label>
                        <select name="trainerId" value={formData.trainerId} onChange={handleChange} className="w-full bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded-lg p-2.5 focus:ring-primary focus:border-primary text-gray-900 dark:text-white" required>
                            <option value="">{t('admin.userManagement.selectPlaceholder')}</option>
                            {trainers.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                        </select>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('classSchedule.modal.date')}</label>
                        ...
                        <div>
                             <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('classSchedule.modal.capacity')}</label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('classSchedule.modal.start')}</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('classSchedule.modal.end')}</label>
