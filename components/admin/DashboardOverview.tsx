...
const StatCard: React.FC<{ title: string; value: number | string; icon: React.ReactNode; colorClass?: string; trend?: string }> = ({ title, value, icon, colorClass = "from-blue-500 to-indigo-500", trend }) => (
  <div className="relative rounded-4xl overflow-hidden bg-white dark:bg-gray-800 shadow-sm border border-black/5 dark:border-white/10 p-8 transition-all hover:shadow-2xl group animate-fade-in">
      <div className="flex justify-between items-start">
        <div className={`p-4 rounded-2xl bg-gradient-to-br ${colorClass} text-white shadow-xl transition-transform group-hover:scale-110 group-hover:rotate-3 duration-500`}>{icon}</div>
        {trend && <span className="text-[10px] font-black text-emerald-500 bg-emerald-500/10 px-2 py-1 rounded-lg">{trend}</span>}
      </div>
      <div className="mt-6">
           <h3 className="text-4xl font-black text-gray-900 dark:text-white tracking-tighter italic">{value}</h3>
    ...
    return (
         <div className="w-full space-y-8 animate-fade-in">
            <div className="flex flex-col md:flex-row justify-between items-end gap-4">
                <div>
                    <h2 className="text-4xl font-black text-gray-900 dark:text-white tracking-tighter uppercase italic">Control de Mando</h2>
                    <p className="text-gray-500 font-medium">Resumen ejecutivo del ecosistema GymPro.</p>
                </div>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="md:col-span-2 relative rounded-4xl overflow-hidden p-8 bg-gray-900 dark:bg-black text-white shadow-2xl group border border-white/5">
                    <SparklesAiIcon className="absolute -right-8 -bottom-8 w-48 h-48 opacity-10 group-hover:scale-110 transition-transform duration-1000 rotate-12" />
                    <div className="relative z-10 space-y-4">
                        <div className="flex items-center gap-2">
                             <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                             <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500">AI STRATEGIC INSIGHT</h3>
                        </div>
                        {loadingAi ? (
                            ...
                            <p className="text-2xl font-bold leading-tight tracking-tight italic">"{aiSummary}"</p>
                        )}
                    </div>
                </div>
                
                <div className="bg-white dark:bg-gray-800 p-8 rounded-4xl border border-black/5 dark:border-white/10 flex flex-col justify-between shadow-sm group hover:shadow-xl transition-all">
                    <div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Capacidad en Vivo</p>
                        <h3 className="text-5xl font-black mt-2 text-gray-900 dark:text-white tracking-tighter italic">{stats.currentAforo} <span className="text-lg text-gray-300 font-medium not-italic">/ {stats.capacity}</span></h3>
                    </div>
                    <div className="mt-6 w-full bg-gray-100 dark:bg-gray-700 h-3 rounded-full overflow-hidden p-0.5 border border-black/5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-white dark:bg-gray-800 p-10 rounded-4xl border border-black/5 dark:border-white/10 shadow-sm relative overflow-hidden">
                    <div className="flex justify-between items-center mb-10">
                        <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter italic">Crecimiento de Membresías</h3>
                    ...
                    <div className="h-[350px]">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={[{n: 'Ene', v: 40}, {n: 'Feb', v: 45}, {n: 'Mar', v: 55}, {n: 'Abr', v: 60}, {n: 'May', v: 80}]} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                ...
                                <Tooltip contentStyle={{ borderRadius: '2rem', border: 'none', boxShadow: '0 25px 50px -12px rgb(0 0 0 / 0.25)', padding: '1.5rem' }} />
                                <Area type="monotone" dataKey="v" stroke="hsl(var(--primary))" fill="url(#colorV)" strokeWidth={6} dot={{ r: 6, fill: 'hsl(var(--primary))', strokeWidth: 3, stroke: '#fff' }} />
                            </AreaChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                <div className="bg-rose-500 rounded-4xl p-1 shadow-2xl">
                    <div className="bg-white dark:bg-gray-900 rounded-[calc(2rem-2px)] p-8 h-full flex flex-col">
                        <h3 className="text-xl font-black text-rose-500 flex items-center gap-3 mb-8 uppercase tracking-tighter italic">
                            <ExclamationTriangleIcon className="w-6 h-6" /> Riesgo de Churn
                        </h3>
                        <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                            {stats.atRisk.map(user => (
                                <div key={user.id} className="bg-gray-50 dark:bg-gray-800/50 p-4 rounded-3xl flex items-center gap-4 border border-black/[0.03] group hover:scale-[1.02] transition-transform">
                                    <img src={user.avatarUrl} className="w-12 h-12 rounded-2xl object-cover border-2 border-white dark:border-gray-700 shadow-sm" />
                                    ...
                                    <button className="p-3 bg-white dark:bg-gray-700 text-primary rounded-2xl shadow-sm hover:bg-primary hover:text-white transition-all">
                            ...
                            {stats.atRisk.length === 0 && (
                                <div className="flex flex-col items-center justify-center h-full opacity-30 text-center py-10">
                                    <CheckCircleIcon className="w-16 h-16 mb-4 text-emerald-500" />
                                    <p className="font-black uppercase tracking-widest text-xs">Máxima Fidelidad</p>
                                </div>
                            )}
                        </div>
                    </div>
